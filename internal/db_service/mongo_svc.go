...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("DISEASE_MONITOR_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("DISEASE_MONITOR_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("DISEASE_MONITOR_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("DISEASE_MONITOR_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("DISEASE_MONITOR_API_MONGODB_DATABASE", "ss-disease-monitor")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("DISEASE_MONITOR_API_MONGODB_COLLECTION", "region")
	}

	if svc.Timeout == 0 {
		seconds := enviro("DISEASE_MONITOR_API_MONGODB_TIMEOUT_SECONDS", "10")
